...
def method_compare(in_file1, in_file2):
    ...
    relative_numeric_tolerance_e_5 = (np.abs(df_merged.iloc[:, 2]-df_merged.iloc[:, 3]) / (1 + np.where(np.abs(df_merged.iloc[:, 2]) < np.abs(df_merged.iloc[:, 3]), np.abs(df_merged.iloc[:, 2]), np.abs(df_merged.iloc[:, 3])))) > 1e-5
    ...
    df_merged = pd.DataFrame(df_merged[is_not_the_same & relative_numeric_tolerance_e_5 & is_too_small_number_x & is_too_small_number_y])
